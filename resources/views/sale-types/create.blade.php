...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('sale-types.store') }}" class="bg-white rounded-lg border border-slate-200">
        ...
        <div class="p-6 space-y-5">
            ...
            <div><label class="block text-sm font-medium text-slate-700 mb-2">Código <span class="text-red-500">*</span></label><input type="text" name="code" value="{{ old('code') }}" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition @error('code') border-red-500 @enderror" placeholder="Ej: pieza">@error('code')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
