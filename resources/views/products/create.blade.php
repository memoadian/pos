...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('products.store') }}" class="bg-white rounded-lg border border-slate-200">
        ...
        <div class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div><label class="block text-sm font-medium text-slate-700 mb-2">Tipo Venta <span class="text-red-500">*</span></label><select name="sale_type" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition"><option value="pieza" {{ old('sale_type') == 'pieza' ? 'selected' : '' }}>Pieza</option><option value="granel" {{ old('sale_type') == 'granel' ? 'selected' : '' }}>Granel</option><option value="peso" {{ old('sale_type') == 'peso' ? 'selected' : '' }}>Peso</option></select></div>
