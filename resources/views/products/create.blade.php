...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('products.store') }}" class="bg-white rounded-lg border border-slate-200">
        ...
        <div class="p-6 space-y-5">
            ...
            @include('products.partials.aliases-field', ['aliases' => old('aliases', [])])
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div><label class="block text-sm font-medium text-slate-700 mb-2">Precio Menudeo <span class="text-red-500">*</span></label><input type="number" name="price_retail" value="{{ old('price_retail', '0.00') }}" step="0.01" min="0" required onfocus="this.select()" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-2">Precio Mayoreo <span class="text-red-500">*</span></label><input type="number" name="price_wholesale" value="{{ old('price_wholesale', '0.00') }}" step="0.01" min="0" required onfocus="this.select()" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition"></div>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div><label class="block text-sm font-medium text-slate-700 mb-2">Precio Super Mayoreo <span class="text-red-500">*</span></label><input type="number" name="price_super_wholesale" value="{{ old('price_super_wholesale', '0.00') }}" step="0.01" min="0" required onfocus="this.select()" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition"></div>
                <div><label class="block text-sm font-medium text-slate-700 mb-2">Costo <span class="text-red-500">*</span></label><input type="number" name="cost" value="{{ old('cost', '0.00') }}" step="0.01" min="0" required onfocus="this.select()" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition"></div>
            </div>
            <div class="border-t border-slate-200 pt-5">
                ...
                <div><label class="block text-sm font-medium text-slate-700 mb-2">Stock Mínimo</label><input type="number" name="min_stock" value="{{ old('min_stock') }}" step="0.01" min="0" placeholder="Ej: 10" onfocus="this.select()" class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition @error('min_stock') border-red-500 @enderror">@error('min_stock')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror</div>
            </div>
            <div class="border-t border-slate-200 pt-5">
