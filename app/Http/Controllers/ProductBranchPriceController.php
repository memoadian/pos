...
class ProductBranchPriceController extends Controller
{
     ...
     *
     * Solo se guarda fila para las sucursales que traen al menos un precio.
     * Si el usuario limpia los tres campos de una sucursal, la fila se borra y
     * esa sucursal vuelve a heredar los precios base del producto.
     */
    public function sync(Request $request, Product $product)
    {
        ...
        try {
            foreach ($allowedBranchIds as $branchId) {
                $input = $validated['prices'][$branchId] ?? [];
